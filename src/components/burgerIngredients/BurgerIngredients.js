...
function BurgerIngredients({ onOpenModal }) {
  ...
  return (
    <section className={burgerIngredients.burgerIngredients}>
      {feedFailed && <h2 className={` ${burgerIngredients.title} text text_type_main-large`}>Произошла ошибка при получении данных</h2>}
      {feedRequest && <h2 className={` ${burgerIngredients.title} text text_type_main-large`}>Загрузка...</h2>}
